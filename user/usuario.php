<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET") 
    {
        ...
        posts.create_at, 
        posts.content, 
        GROUP_CONCAT(midia.media_url SEPARATOR ':') AS img_urls
    FROM 
        users
    LEFT JOIN 
        posts ON users.id = posts.user_id
    LEFT JOIN 
        midia ON posts.id = midia.post_id
    WHERE 
        users.id = :user_id
    GROUP BY 
        users.id, posts.id;";
        $consulta = $pdo->prepare($query);
        $consulta->bindParam(":user_id", $_GET["fuy"], PDO::PARAM_STR);
...
<body>
    ...
    <div class="container">
        ...
        <form action="" method="post" enctype="multipart/form-data" class="ADDPost">
                <div class="perfil">
                    <img src="<?php echo $users[0]->profile_pic; ?>" alt="Foto de perfil">
                    <div class="info">
                        <h2><?php echo $users[0]->name; ?></h2>
                        <p><?php echo $users[0]->bios;?></p>
                        <a href="<?php echo $users[0]->link_profile;?>" target="_blank"><i class="fa-solid fa-link"></i><?php echo $users[0]->link_profile?></a>
                    </div>
                </div>
                <nav>
